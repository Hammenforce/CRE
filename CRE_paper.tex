...
This work introduces Complex Resonance Embedding (CRE), an architecture achieving O($n$) complexity through a fundamentally different computational primitive: gated cumulative operations with position modulation. Rather than computing pairwise interactions between all positions, CRE maintains running states that accumulate information selectively. The key innovations are:

\begin{enumerate}
\item \textbf{Push-pull cumulative dynamics}: Two complementary cumulative sums---push (accumulation) and pull (subtraction)---whose difference captures temporal patterns through differential aggregation.

\item \textbf{Position-modulated output}: Learnable exponential weighting based on sequence position normalizes cumulative sums and creates adaptive position-dependent patterns.

\item \textbf{Multi-frequency resonance}: Parallel processing at multiple modulation rates enables simultaneous modeling of different temporal scales.
...
The core computation maintains two cumulative states. At position $t$:
...
\textbf{Cumulative Accumulation:}
\begin{align}
P_t &= \sum_{i=1}^{t} g_i \odot v_i \quad \text{(push accumulator)} \\
Q_t &= \sum_{i=1}^{t} (1-g_i) \odot v_i \quad \text{(pull accumulator)}
\end{align}
where $\odot$ denotes element-wise multiplication.

\textbf{Position-Modulated Resonance Output:}
\begin{equation}
h_t = e^{-\lambda t} \cdot (P_t - Q_t)
\end{equation}

The push state accumulates information weighted by the gate, while the pull state accumulates the complement. Their difference produces a resonance pattern reflecting the balance of accumulated information. The position modulation factor $e^{-\lambda t}$ applies a learnable decay based on absolute position, which serves to normalize the growing cumulative sums and create a position-dependent weighting that the model can optimize.

\subsection{Vectorized Implementation}

For efficient parallel computation, the mechanism is implemented using vectorized cumulative sums:
\begin{align}
\text{push}_t &= g_t \odot v_t \\
\text{pull}_t &= (1 - g_t) \odot v_t \\
P_t &= \text{cumsum}(\text{push})_t \\
Q_t &= \text{cumsum}(\text{pull})_t \\
h_t &= e^{-\lambda t} \cdot (P_t - Q_t)
\end{align}

This is computed efficiently as:
\begin{equation}
(\text{cumsum}(\text{push}) - \text{cumsum}(\text{pull})) \cdot \text{decay\_weights}
\end{equation}
where decay\_weights $= [e^{-\lambda \cdot 0}, e^{-\lambda \cdot 1}, \ldots, e^{-\lambda \cdot (L-1)}]$ are precomputed exponential factors. The entire operation is O($L \cdot d$) in time and O($L \cdot d$) in memory, with no sequential loops required.
